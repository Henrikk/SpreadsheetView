...
public class SpreadsheetView: UIView {
    ...
    // special functions
       ...
       public func clearHighLighted(){
           self.highlightedIndexPaths.removeAll()
       }
       
       public func addHighLight(dexPath: IndexPath){
           self.highlightedIndexPaths.insert(dexPath)
       }
      
    
    public func reloadData_2(index1: IndexPath,index2: IndexPath) {
        do{
            ...
            resetContentSize_2(of: tableView,index1: index1, index2: index2)
    ...
    func resetContentSize_2(of scrollView: ScrollView, index1: IndexPath, index2: IndexPath) {
        defer {
        ...
        let startColumn = index1.column//scrollView.layoutAttributes.startColumn
        let columnCount = index2.column  - startColumn//scrollView.layoutAttributes.columnCount
        var width: CGFloat = 0
        print(startColumn, columnCount)
        for column in startColumn..<index2.column {
    ...
    public func dequeueReusableCell(withReuseIdentifier identifier: String, for indexPath: IndexPath) -> Cell {
       ...
       // fatalError("could not dequeue a view with identifier cell - must register a nib or a class for the identifier")
        // Fallback: Log an error and return a default cell instead of crashing
            print("⚠️ Warning: No cell found for identifier '\(identifier)'. Returning a default BlankCell.")
            
            let defaultCell = BlankCell()
            defaultCell.reuseIdentifier = identifier
            return defaultCell
    }
    ...
    public func scrollToItem(at indexPath: IndexPath, at scrollPosition: ScrollPosition, animated: Bool) {
        do{
            let contentOffset = try contentOffsetForScrollingToItem(at: indexPath, at: scrollPosition)
   ...
   /* private func contentOffsetForScrollingToItem(at indexPath: IndexPath, at scrollPosition: ScrollPosition) -> CGPoint {
    ...
    private enum ScrollError: Error {
        case invalidIndexPath(column: Int, row: Int)
        case multipleHorizontalScrollPositions
        case multipleVerticalScrollPositions
    }

    private func contentOffsetForScrollingToItem(at indexPath: IndexPath, at scrollPosition: ScrollPosition) throws -> CGPoint {
        let (column, row) = (indexPath.column, indexPath.row)
        
        // Guard against invalid index path
        guard column < numberOfColumns && row < numberOfRows else {
            throw ScrollError.invalidIndexPath(column: column, row: row)
        }
        ...
        // Handle circular scrolling horizontally
        if circularScrollingOptions.direction.contains(.horizontally) {
            contentOffset.x = contentOffset.x > centerOffset.x ? contentOffset.x - centerOffset.x : contentOffset.x + centerOffset.x
        }
        ...
        if scrollPosition.contains(.centeredHorizontally) {
            horizontalGroupCount += 1
            contentOffset.x = max(
                tableView.contentOffset.x + (contentOffset.x - (tableView.contentOffset.x + (tableView.frame.width - (width + intercellSpacing.width * 2)) / 2)),
                0
            )
        }
        if scrollPosition.contains(.right) {
        ...
        // Handle circular scrolling vertically
        if circularScrollingOptions.direction.contains(.vertically) {
            contentOffset.y = contentOffset.y > centerOffset.y ? contentOffset.y - centerOffset.y : contentOffset.y + centerOffset.y
        }
        ...
        if scrollPosition.contains(.centeredVertically) {
            verticalGroupCount += 1
            contentOffset.y = max(
                tableView.contentOffset.y + contentOffset.y - (tableView.contentOffset.y + (tableView.frame.height - (height + intercellSpacing.height * 2)) / 2),
                0
            )
        }
        if scrollPosition.contains(.bottom) {
        ...
        // Adjust content offset if too close to the edges
        let distanceFromRightEdge = tableView.contentSize.width - contentOffset.x
        if distanceFromRightEdge < tableView.frame.width {
            contentOffset.x -= tableView.frame.width - distanceFromRightEdge
        }
        ...
        // Check for invalid multiple scroll positioning styles
        if horizontalGroupCount > 1 {
            throw ScrollError.multipleHorizontalScrollPositions
        }
        if verticalGroupCount > 1 {
            throw ScrollError.multipleVerticalScrollPositions
        }

        // Ensure content offset is non-negative
