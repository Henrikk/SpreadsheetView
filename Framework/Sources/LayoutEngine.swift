...
final class LayoutEngine {
   ...
   */
    ...
    enum LayoutError: Error, CustomStringConvertible {
        case cellCreationFailed(IndexPath)
        case missingReuseIdentifier(IndexPath)

        var description: String {
            switch self {
            case .cellCreationFailed(let indexPath):
                return "Failed to create or dequeue a cell at \(indexPath)"
            case .missingReuseIdentifier(let indexPath):
                return "Cell at \(indexPath) is missing a reuseIdentifier"
            }
        }
    }
    ...
    private func layoutCell(address: Address, frame: CGRect) {
        do {
            try _layoutCell(address: address, frame: frame)
        } catch {
            print("⚠️ Failed to layout cell at address \(address): \(error)")
        }
    }
    
   
    
    private func _layoutCell(address: Address, frame: CGRect) throws {
        guard let dataSource = spreadsheetView.dataSource else { return }

        var gridlines: Gridlines?
        var border: (borders: Borders?, hasBorders: Bool)
        ...
        if let cell = scrollView.visibleCells[address] {
            ...
            let indexPath = IndexPath(row: address.row, section: address.column)

            let cell: Cell
            if let providedCell = dataSource.spreadsheetView(spreadsheetView, cellForItemAt: indexPath) {
                cell = providedCell
            } else {
                cell = spreadsheetView.dequeueReusableCell(withReuseIdentifier: blankCellReuseIdentifier, for: indexPath)
            }

            guard cell.reuseIdentifier != nil else {
                throw LayoutError.missingReuseIdentifier(indexPath)
            }
    ...
    private func old_layoutCell(address: Address, frame: CGRect) {
        guard let dataSource = spreadsheetView.dataSource else {
            return
        }
        ...
        if scrollView.visibleCells.contains(address) {
            ...
            // Try to dequeue a reusable cell from the data source, fallback to a blank cell if needed
            let cell = dataSource.spreadsheetView(spreadsheetView, cellForItemAt: indexPath) ??
                       spreadsheetView.dequeueReusableCell(withReuseIdentifier: blankCellReuseIdentifier, for: indexPath)
            
            // Ensure the dequeued or provided cell has a reuseIdentifier
            guard let _ = cell.reuseIdentifier else {
                print("Error: The cell returned from `spreadsheetView(_:cellForItemAt:)` does not have a `reuseIdentifier`. Cells must be retrieved by calling `dequeueReusableCell(withReuseIdentifier:for:)`.")
                return // Early return if reuseIdentifier is missing
            }
        ...
        // Add the address to visible borders if the cell has borders
        if border.hasBorders {
            visibleBorderAddresses.insert(address)
        }
        
        // Layout gridlines if available
        if let gridlines = gridlines {
            layoutGridlines(address: address, frame: frame, gridlines: gridlines)
        }
    }
