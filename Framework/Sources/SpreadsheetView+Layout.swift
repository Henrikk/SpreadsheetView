...
extension SpreadsheetView {
  ...
  /*  func resetLayoutProperties() -> LayoutProperties {
    ...
    enum LayoutError: Error {
        ...
        case frozenColumnsExceedTotal
        case frozenRowsExceedTotal
        case invalidMergedCells
        case cellRangeExceedsBounds
        case overlappingMergedCells
    }
    ...
    func resetLayoutProperties() throws -> LayoutProperties {
        ...
        // Validate column and row counts
        guard numberOfColumns >= 0 else {
            throw LayoutError.invalidColumnCount
        }
        guard numberOfRows >= 0 else {
            throw LayoutError.invalidRowCount
        }

        // Validate frozen columns and rows
        guard frozenColumns <= numberOfColumns else {
            throw LayoutError.frozenColumnsExceedTotal
        }
        guard frozenRows <= numberOfRows else {
            throw LayoutError.frozenRowsExceedTotal
        }
        ...
        let mergedCellLayouts: [Location: CellRange] = {
            var layouts = [Location: CellRange]()
            for mergedCell in mergedCells {
                // Check if frozen and non-frozen rows or columns are merged
                if (mergedCell.from.column < frozenColumns && mergedCell.to.column >= frozenColumns) ||
                    (mergedCell.from.row < frozenRows && mergedCell.to.row >= frozenRows) {
                    // Throw an error if merging across frozen/non-frozen boundaries
                    print("Warning: cannot merge frozen and non-frozen columns or rows")
                    continue
                }
                // Validate merged cell range
                for column in mergedCell.from.column...mergedCell.to.column {
                    for row in mergedCell.from.row...mergedCell.to.row {
                        guard column < numberOfColumns && row < numberOfRows else {
                            print("Warning: merged cell range exceeds column/row count. Column:", column, "Number of Columns:", numberOfColumns)
                            return layouts
                        }
                        let location = Location(row: row, column: column)
                        if let existingMergedCell = layouts[location] {
                            // Prevent overlapping merged cells
                            if existingMergedCell.contains(mergedCell) {
                                continue
                            }
                            if mergedCell.contains(existingMergedCell) {
                                ...
                                print("Warning: overlapping merged cells detected")
                                return layouts
                            }
                        }
                        mergedCell.size = nil
